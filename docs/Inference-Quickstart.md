...
This quickstart shows how to (1) initialize the SpaceNit model, (2) obtain a satellite
...
```bash
uv run --with spacenit --with torch --python 3.12 - <<'EOF'
import torch
from spacenit.checkpoint_loader import PretrainedModelID, load_pretrained
from spacenit.structures import MaskedGeoSample, TokenVisibility

# Load model from HuggingFace
model = load_pretrained(PretrainedModelID.SPACENIT_V1_BASE)
model.eval()

# Create synthetic input (B=1, H=64, W=64, T=1, C=12 for Sentinel-2)
dummy_image = torch.randn(1, 64, 64, 1, 12)
dummy_mask = torch.ones(1, 64, 64, 1, 3) * TokenVisibility.VISIBLE_ENCODER.value
...
Sentinel-2 images, see the SpaceNit embedding and fine-tuning guides.

To download an image from the Copernicus Browser, follow these steps:
...
2. Go to the Search tab at the top-left. Check Sentinel-2, then check L2A. This selects
   Sentinel-2 L2A images, which are the type of Sentinel-2 images that SpaceNit is
   pre-trained on.
...
```
wget https://storage.googleapis.com/spacenit-public-data/artifacts/example_sentinel2_l2a_scene_of_seattle.zip
...
import numpy as np
import rasterio
from spacenit.ingestion.modalities import Sensor
from rasterio.vrt import WarpedVRT
from rasterio.enums import Resampling

# Get the JP2 filenames that we need to read, in the channel order expected by SpaceNit.
fnames = []
for channel_name in Sensor.SENTINEL2_L2A.all_channel_names:
    fname = glob.glob(f"*.SAFE/GRANULE/*/IMG_DATA/*/*_{channel_name}_*.jp2")[0]
    fnames.append(fname)
...
sample = MaskedGeoSample(
    ...
    # The mask shape is BHWTS, where S is the number of spectral groups (3 for Sentinel-2).
    sentinel2_l2a_mask=torch.ones((1, 64, 64, 1, 3), dtype=torch.float32, device=device) * TokenVisibility.VISIBLE_ENCODER.value,
...
# Pool the features over the timestep and spectral group dimensions so we end up with a
# BHWC feature map.
