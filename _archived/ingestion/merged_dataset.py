"""Merged (concatenated) dataset for the SpaceNit ingestion pipeline."""
...
from .tile_dataset import GetItemArgs

logger = logging.getLogger(__name__)


class GeoTileMergedDataset(ConcatDataset):
    """Dataset based on ConcatDataset for concatenating multiple GeoTileDatasets.

    The resulting GeoTileMergedDataset acts as a concatenated version of the
    individual GeoTileDatasets.

    We need to use a custom GeoTileMergedDataset because we have a custom way
    to access __getitem__ (instead of just integer index), and we need to
    support various functions and attributes expected by the GeoTileLoader and
    various callbacks.
    """
    ...
    def __getitem__(self, args: GetItemArgs) -> Any:
        ...
        # The only change we make is to extract the index from args, and then
        # get a tuple with updated index at the end to pass to the sub dataset.
        idx = args.idx
        if idx < 0:
            if -idx > len(self):
                raise ValueError(
    ...
    def fingerprint_version(self) -> str:
        ...
        for dataset in self.datasets:
            if dataset.fingerprint_version != version:
                raise ValueError(
                    "expected all sub datasets to have the same "
                    "fingerprint_version"
                )
        return version
    ...
    def _set_latlon_distribution(self) -> None:
        """Set the latlon distribution based on the sub datasets."""
        dataset_latlons = []
        for dataset in self.datasets:
    ...
    def prepare(self) -> None:
        ...
        # GeoTileMergedDataset since otherwise they would not have a length,
        # but we prepare here just in case.
        for dataset in self.datasets:
            dataset.prepare()

        # We need to compute latlon_distribution attribute since it is expected
        # by some callback.
...
@dataclass
class GeoTileMergedDatasetConfig(Config):
    """Configuration for the GeoTileMergedDataset."""
    ...
    def build(self) -> GeoTileMergedDataset:
        """Build the dataset."""
        self.validate()
        ...
        for dataset_config in self.dataset_configs:
            ...
            # Dataset must be prepared before passing to
            # GeoTileMergedDataset so it has a defined length.
            dataset.prepare()
            datasets.append(dataset)
        return GeoTileMergedDataset(datasets)
