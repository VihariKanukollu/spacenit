...
from spacenit.ingestion.modalities import Sensor, SensorSpec, TimeSpan

from .helpers import get_modality_dir, get_modality_temp_meta_dir
...
def make_meta_summary(
    ...
    Args:
        spacenit_path: SpaceNit dataset path.
    ...
    meta_dir = get_modality_temp_meta_dir(spacenit_path, modality, time_span)
    ...
    with (spacenit_path / f"{modality_dir.name}.csv").open("w") as f:
...
if __name__ == "__main__":
    ...
    modality = Sensor.by_label(args.modality)
    make_meta_summary(UPath(args.spacenit_path), modality, TimeSpan(args.time_span))
