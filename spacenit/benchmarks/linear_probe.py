...
from spacenit.benchmarks.datasets.registry import BenchmarkDatasetConfig, TaskType
from spacenit.benchmarks.scoring import (
    BenchmarkResult,
    BenchmarkTaskResult,
    segmentation_metrics,
)
from spacenit.benchmarks.helpers import adjust_learning_rate
...
def train_and_eval_probe(
    ...
    """Run a linear probe on the SpaceNit model.
    ...
    if train_features.shape[-1] != val_features.shape[-1]:
        raise ValueError("Feature dims don't match.")
    ...
    if config.task_type == TaskType.SEGMENTATION:
        ...
        num_patches = train_features.shape[1] * train_features.shape[2]
    ...
    val_results: list[BenchmarkResult] = []
    ...
    if test_features is not None:
        ...
        logger.info(
            f"Computing predictions for {test_features.shape[0]} test samples..."
        )
...
def get_probe_predictions(
    ...
    if probe_type == ProbeType.ATTNPOOL:
        ...
        per_head = all_attn_weights_tensor.mean(dim=(0, 2))
        overall = all_attn_weights_tensor.mean(dim=(0, 1, 2))
...
def compute_metric(
    ...
    if task_type == TaskType.SEGMENTATION:
        ...
        return BenchmarkResult.from_classification(acc)
