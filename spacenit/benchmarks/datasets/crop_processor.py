"""Script to process the PASTIS crop parcels dataset."""
...
class CropParcelsProcessor:
    ...
    def __init__(self, data_dir: str, output_dir: str, resize_to_64: bool = True):
        """Initialize crop parcels processor.
        ...
        for idx, date in dates.items():
            ...
            if modality_name == SENTINEL2_L2A.label:
                if img.shape[0] == 10:
                    ...
                    raise ValueError(
                        f"Sentinel2 image has {img.shape[0]} bands, expected 10!"
                    )
...
def process_crop_parcels(data_dir: str, output_dir: str) -> None:
    """Process PASTIS-R dataset."""
    processor = CropParcelsProcessor(
        data_dir=data_dir,
...
def process_crop_parcels_orig_size(
    data_dir: str,
    output_dir: str,
) -> None:
    """Process PASTIS-R dataset at original size."""
    processor = CropParcelsProcessor(
...
def main() -> None:
    ...
    if args.orig_size:
        process_crop_parcels_orig_size(data_dir=args.data_dir, output_dir=args.output_dir)
    else:
        process_crop_parcels(data_dir=args.data_dir, output_dir=args.output_dir)
