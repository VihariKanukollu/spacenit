"""Flood scenes eval dataset, based on Sen1Floods11."""
...
class FloodScenesProcessor:
    ...
    def split_and_filter_tensors(
        ...
        """Split image and label tensors into tiles and filter based on label content."""
        assert s1.shape == (
            len(cls.s1_bands),
            cls.input_hw,
            cls.input_hw,
...
def get_flood_scenes(
    ...
    """Calls FloodScenesProcessor to preprocess the floods dataset."""
    split_path = flood_folder / split_name
    dataset = FloodScenesProcessor(folder=flood_folder, split_path=split_path)
...
class FloodScenesDataset(Dataset):
    ...
    def __init__(
        ...
        self.s1, self.labels = self._remove_nan(self.s1, self.labels)
