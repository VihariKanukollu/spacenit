"""Crop timeseries (Breizhcrops) benchmark dataset."""
...
def _spacenit2bc_name(band_name: str) -> str:
    """Transform SpaceNit S2 band name to Breizhcrops S2 band name."""
...
class CropTimeseriesDataset(Dataset):
    """The Breizhcrops crop timeseries dataset."""
    ...
    def __init__(
        ...
        Args:
            path_to_splits: Path where breizhcrops data is stored
            split: Split to use
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "CropTimeseries eval to run."
            )

        self.bc_selected_bands = SELECTED_BANDS

        self.input_to_output_band_mapping = [
            SELECTED_BANDS[LEVEL].index(_spacenit2bc_name(b))
            for b in BENCH_S2_BAND_NAMES
        ]
        kwargs = {
            "root": path_to_splits,
            "preload_ram": False,
