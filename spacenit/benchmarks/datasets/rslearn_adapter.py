"""Convert rslearn dataset to SpaceNit benchmark dataset format."""
...
def build_rslearn_model_dataset(
    ...
    Args:
        ...
        skip_targets: Whether or not to skip the target.
    ...
    if not layers:
        raise ValueError(
            ...
            f"allowed: {list(RSLEARN_TO_SPACENIT.keys())}"
        )
    if split not in ("train", "val", "test"):
        raise ValueError(f"Invalid split {split}, must be one of train/val/test")

    # Validate input layers
    unknown = [m for m in layers if m not in RSLEARN_TO_SPACENIT]
    if unknown:
        raise ValueError(
    ...
    # Group rslearn layers by their SpaceNit modality key
    layers_by_spacenit: dict[str, list[str]] = defaultdict(list)
    bands_by_spacenit: dict[str, list[str]] = {}

    for rslearn_layer in layers:
        spacenit_key, band_order = RSLEARN_TO_SPACENIT[rslearn_layer]
        layers_by_spacenit[spacenit_key].append(rslearn_layer)
        bands_by_spacenit[spacenit_key] = band_order
    ...
    if input_size is not None:
        transforms.append(
            RsPad(
                size=input_size,
                mode="center",
                image_selectors=list(layers_by_spacenit.keys()),
            )
        )

    inputs: dict[str, RsDataInput] = {}
    # Expand each rslearn layer name to time-indexed variants
    for spacenit_key, per_key_layers in layers_by_spacenit.items():
        expanded: list[str] = []
        ...
        inputs[spacenit_key] = RsDataInput(
            data_type="raster",
            layers=expanded,
            bands=bands_by_spacenit[spacenit_key],
            passthrough=True,
...
class RslearnToSpaceNitDataset(Dataset):
    ...
    allowed_modalities = {
        DataModality.SENTINEL2_L2A.label,
        DataModality.SENTINEL1.label,
        DataModality.LANDSAT.label,
    }
    ...
    def _get_norm_stats(ds_norm_stats_json: str) -> dict:
        ...
        with (
            files("spacenit.benchmarks.datasets.config") / ds_norm_stats_json
        ).open() as f:
            blob = json.load(f)
        out = {}
        for modality, per_band in blob.items():
            band_order = DataModality.by_label(modality).all_channel_names
    ...
    def __getitem__(self, idx: int) -> tuple[MaskedSpaceNitSample, torch.Tensor]:
        ...
        for modality in self.input_modalities:
            ...
            num_bands = DataModality.by_label(modality).total_channels
            ...
            if modality == DataModality.SENTINEL1.label:
                x = convert_to_db(x)
            x = rearrange(x, "(t c) h w -> h w t c", t=T, c=num_bands)

            if self.norm_stats_from_pretrained:
                x = self.normalizer_computed.normalize(DataModality.by_label(modality), x)
            else:
