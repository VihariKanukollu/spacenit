"""GeoBench datasets, returning data in the SpaceNit format."""
...
from .band_scaling import impute_normalization_stats, normalize_bands
...
def _landsat_spacenit2geobench_name(band_name: str) -> str:
    """Transform SpaceNit Landsat band name to Geobench Landsat band name."""
    transform = {
        ...
        "B6": "06 - SWIR1",
        "B7": "07 - SWIR2",
        # B8 is the panchromatic band. Geobench does not include it.
...
GEOBENCH_L8_BAND_NAMES = [
    _landsat_spacenit2geobench_name(b) for b in BENCH_L8_BAND_NAMES
]


class GeoBenchmarkDataset(Dataset):
    """GeoBench dataset, returning data in the SpaceNit format."""
    ...
    def __init__(
        ...
        if split not in ["train", "valid", "test"]:
            raise ValueError(
                f"Expected split to be in ['train', 'valid', 'test'], got {split}"
            )
        ...
        imputed_band_info = impute_normalization_stats(
            task.band_stats,
            config.imputes,
            all_bands=GEOBENCH_L8_BAND_NAMES if self.is_landsat else BENCH_S2_BAND_NAMES,
        )
    ...
    def __getitem__(self, idx: int) -> tuple[MaskedGeoSample, torch.Tensor]:
        ...
        if self.is_landsat:
            assert x.shape[-1] == len(BENCH_L8_BAND_NAMES), (
                f"Instances must have {len(BENCH_L8_BAND_NAMES)} channels, not {x.shape[-1]}"
            )
        else:
            assert x.shape[-1] == len(BENCH_S2_BAND_NAMES), (
                f"Instances must have {len(BENCH_S2_BAND_NAMES)} channels, not {x.shape[-1]}"
            )
        if self.multiply_by_10_000:
            x = x * 10_000
    ...
    def visualize_sample_bands(self, x: np.ndarray, output_dir: str) -> None:
        """Visualize each band from a given array, saving each plot as a PNG file.
