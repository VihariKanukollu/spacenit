"""KNN evaluation of SpaceNit models."""
...
from spacenit.benchmarks.datasets.registry import BenchmarkDatasetConfig
...
def run_knn(
    ...
    """Run KNN evaluation on the SpaceNit model.
    ...
    Returns:
        BenchmarkTaskResult with val_result, test_result, and bootstrap_stats.
    """
    ...
    if not config.is_multilabel:
        ...
        if test_features is not None:
            if test_labels is None:
                raise ValueError("Can't have test features without test labels")
            ...
            test_result = BenchmarkResult.from_classification(test_score)
        ...
        return BenchmarkTaskResult(
            val_result=BenchmarkResult.from_classification(val_score),
            test_result=test_result,
            bootstrap_stats=bootstrap_stats,
        )
    else:
        # multilabel dataset, e.g., BigEarthNet
        # we will run KNN once per class to compute predictions
...
def _bootstrap_knn_test(
    ...
    logger.info(
        f"Computing predictions once for {test_features.shape[0]} test samples..."
    )
...
def _run_knn_for_k(
    ...
    Args:
        train_features: Training features of shape (n_train, feature_dim)
        train_labels: Training labels of shape (n_train,)
        test_features: Test features of shape (n_test, feature_dim)
    ...
    eps = 1e-8
    train_features_norm = torch.nn.functional.normalize(
        train_features, p=2, dim=1, eps=eps
    )

    # Process test features in chunks to avoid memory explosion
    ...
    for start_idx in range(0, n_test, chunk_size):
        ...
        similarities = torch.mm(test_chunk_norm, train_features_norm.T)
