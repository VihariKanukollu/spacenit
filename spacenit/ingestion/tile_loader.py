...
from spacenit.ingestion.augmentations import Transform, TransformConfig
from spacenit.ingestion.batch_assembly import (
    collate_double_masked_batched,
    collate_single_masked_batched,
)
from spacenit.ingestion.merged_dataset import GeoTileMergedDataset
from spacenit.ingestion.sensors import TILE_EDGE_PIXELS, SensorRegistry
from spacenit.ingestion.tile_dataset import (
    GetItemArgs,
    GeoTileDataset,
)
...
# ---------------------------------------------------------------------------
# Masking types – imported lazily to avoid hard dependency on the pipeline
...
class GeoTileLoader(DataLoaderBase):
    """SpaceNit geo-tile dataloader.

    This dataloader is adapted from OLMo-core's TextDataLoaderBase and
    NumpyDataLoaderBase, incorporating their core functionality for DDP,
    multi-threading, and multi-processing.
    """

    def __init__(
        self,
        dataset: GeoTileDataset | GeoTileMergedDataset,
        work_dir: UPath,
        ...
        masking_strategy: Any | None = None,
        masking_strategy_b: Any | None = None,
        num_masked_views: int = 1,
        tokenization_config: TokenizationConfig | None = None,
    ):
        """Initialize the GeoTileLoader.
        ...
        Args:
            ...
            sampled_hw_p_list: List of possible height/width in patches to
                sample.
            ...
            multiprocessing_context: Multiprocessing context ("spawn" or
                "forkserver").
            num_dataset_repeats_per_epoch: Number of times to repeat the
                dataset per epoch.
            transform: Optional transform to apply in the dataloader workers.
            masking_strategy: Masking strategy to apply in the dataloader
                workers.
            masking_strategy_b: Optional second masking strategy for
                dual-branch training.
            num_masked_views: Number of masked views to return (1=single,
                2=double).
            tokenization_config: Optional tokenization config for custom band
                groupings.
        """
        super().__init__(
            work_dir=work_dir,
        ...
        if num_masked_views not in (1, 2):
            raise ValueError(
                f"num_masked_views must be 1 or 2, got {num_masked_views}"
            )
        ...
        if in_memory:
            ...
            if self.fs_local_rank == 0:
                ...
                if self._global_indices_file.is_file():
                    logger.info(
                        f"Using existing global indices file for seed "
                        f"{self.seed} and epoch {self.epoch}"  # type: ignore
    ...
    def get_global_indices(self) -> np.ndarray:
        ...
        if not self._global_indices_file.is_file():
            raise RuntimeError(
                f"Missing global indices file {self._global_indices_file}, "
                "did you forget to call 'reshuffle()'?"
            )
        return np.memmap(self._global_indices_file, mode="r", dtype=np.uint32)

    def _iter_batches(self) -> Iterable[GeoSample]:
        ...
        return torch.utils.data.DataLoader(
            ...
            pin_memory=(
                self.target_device_type == "cuda" and self.num_workers > 0
            ),
            prefetch_factor=(
    ...
    def load_state_dict(self, state_dict: dict[str, Any]) -> None:
        ...
        if (
            ...
            logger.warning(
                "Dataset fingerprint version does not match the version in "
                "the checkpoint, this could mean the data has changed"
            )
        elif state_dict["dataset_fingerprint"] != self.dataset.fingerprint:
            logger.warning(
                "Restoring state from a different dataset! If this is not "
                "expected, please check the dataset fingerprint (fingerprint "
                "doesn't match). "
                f"old fingerprint: {state_dict['dataset_fingerprint']}, "
                f"new fingerprint: {self.dataset.fingerprint}"
            )

        if state_dict["seed"] != self.seed:
            logger.warning(
                "Restoring data loading state with a different data seed, "
                "will use data seed from state dict for data order "
                "consistency."
            )
    ...
    def get_mock_batch(self) -> Any:
        ...
        - 1: (patch_size, MaskedGeoSample) - single masked view
        - 2: (patch_size, MaskedGeoSample, MaskedGeoSample) - double masked
        """
    ...
    def fast_forward(self, global_step: int) -> np.ndarray:
        ...
        logger.warning(
            "Fast forward does not yet support returning to indices for "
            "multiple GPUs"
        )
        if get_world_size() > 1:
            raise NotImplementedError("Fast forward is not supported in DDP")
        # If the model was trained with multiple GPUs, this logic must be
        # updated so that we grab from where all the ranks started.
...
def iter_batched(
    iterable: Iterable[tuple[int, GeoSample]],
    batch_size: int,
    drop_last: bool = True,
) -> Iterable[tuple[tuple[int, GeoSample], ...]]:
    """Iterate over the dataset in batches.

    This is a modified version of olmo_core.data.data_loader.iter_batched that
    creates batches of size local_batch_size for the local rank from an
    iterator of items.
...
class _IterableDatasetWrapper(torch.utils.data.IterableDataset[GeoSample]):
    """Iterable dataset wrapper.

    This is a modified version of
    olmo_core.data.data_loader._IterableDatasetWrapper.
    """

    def __init__(self, data_loader: GeoTileLoader):
    ...
    def _get_batch_item_params_iterator(
        ...
        worker_id = (
            self.worker_info.id if self.worker_info is not None else 0
        )
        ...
        for idx in indices:
            if instances_processed % rank_batch_size == 0:
                ...
                max_height_width_tokens = int(TILE_EDGE_PIXELS / patch_size)
    ...
    def __iter__(self) -> Iterator[Any]:
        ...
        Transform and masking are applied in the batched collator for better
        vectorization.
        """
        ...
        instance_iterator = (
            self.data_loader._get_dataset_item(
                int(idx), patch_size, sampled_hw_p
            )
...
@dataclass
class GeoTileLoaderConfig(Config):
    """Configuration for the GeoTileLoader."""
    ...
    def validate(self) -> None:
        ...
        if self.num_masked_views not in (1, 2):
            raise ValueError(
                f"num_masked_views must be 1 or 2, got "
                f"{self.num_masked_views}"
            )
    ...
    def build(
        ...
        """Build the GeoTileLoader."""
        self.validate()
        dataset.prepare()

        # Build transform and masking strategies
        transform = (
            self.transform_config.build()
        ...
        # batch at once for better vectorization.
        collator: Callable
        ...
        return GeoTileLoader(
            ...
            target_device_type=(
                self.target_device_type or get_default_device().type
            ),
            collator=collator,
