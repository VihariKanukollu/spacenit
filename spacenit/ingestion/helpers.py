"""Utility helpers for the SpaceNit ingestion pipeline."""
...
def update_streaming_stats(
    ...
    Args:
        ...
        sensor_band_data: The data for the current sensor band.

    Returns:
        Updated count, mean, and variance for the sensor band.
    """
    band_data_count = np.prod(sensor_band_data.shape)
    ...
    new_mean = (
        current_mean
        + (sensor_band_data.mean() - current_mean) * band_data_count / new_count
    )
    new_var = (
        current_var
        + ((sensor_band_data - current_mean) * (sensor_band_data - new_mean)).sum()
    )
...
def plot_sensor_data_distribution(sensor_label: str, sensor_data: dict) -> plt.Figure:
    """Plot the data distribution for a given sensor.

    Args:
        sensor_label: Human-readable sensor name.
        sensor_data: Mapping from band name to an array of values.

    Returns:
        A matplotlib Figure with one histogram per band.
    """
    fig, axes = plt.subplots(
        len(sensor_data), 1, figsize=(10, 5 * len(sensor_data))
    )
    if len(sensor_data) == 1:
        axes = [axes]
    for ax, (band, values) in zip(axes, sensor_data.items()):
        ax.hist(values, bins=50, alpha=0.75)
        ax.set_title(f"{sensor_label} - {band}")
        ax.set_xlabel("Value")
