...
class Sensor:
    """Backward-compatible shim that delegates to :class:`SensorRegistry`.

    Prefer using :class:`SensorRegistry` directly in new code.
    """

    # Expose every built-in sensor as a class attribute for compatibility
    NAIP = NAIP
    NAIP_10 = NAIP_10
    SENTINEL1 = SENTINEL1
    SENTINEL2 = SENTINEL2
    ...
    CDL = CDL
    WORLDPOP = WORLDPOP
    WRI_CANOPY_HEIGHT_MAP = WRI_CANOPY_HEIGHT_MAP

    @classmethod
    def by_label(cls, label: str) -> SensorSpec:
        return SensorRegistry.get(label)

    @classmethod
    def all_specs(cls) -> list[SensorSpec]:
        return SensorRegistry.all_specs()
...
def specs_from_labels(labels: list[str]) -> list[SensorSpec]:
    """Look up multiple :class:`SensorSpec` instances by their labels."""
    return [SensorRegistry.get(lbl) for lbl in labels]
